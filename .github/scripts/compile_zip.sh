...
if [ -d "daemon/libs" ]; then
	echo "Copying daemon libs..."
	cp -rn daemon/libs/* module/libs/
else
	echo "Daemon libs not found!" >&2
	exit 1
fi

# Copy profiler build output
if [ -d "profiler/libs" ]; then
	echo "Copying profiler libs..."
	cp -rn profiler/libs/* module/libs/
else
	echo "Profiler libs not found!" >&2
	exit 1
fi
