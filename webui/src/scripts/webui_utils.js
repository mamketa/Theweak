...
const saveLog = async () => {
  const output = await runCommand(`${binPath}/nusantara_utility save_logs`);
  if (output.error) {
...
const changeCPUGovernor = async (governor, config) => {
  ...
  if (config === "powersave_cpu_gov") {
    await runCommand(`[ "$(<${configPath}/current_profile)" -eq 3 ] && nusantara_utility change_cpu_gov ${governor}`);
  } else if (config === "custom_default_cpu_gov") {
    await runCommand(`[ "$(<${configPath}/current_profile)" -eq 2 ] && nusantara_utility change_cpu_gov ${governor}`);
  }
};
...
document.getElementById('nusantara_logo').addEventListener('click', () => openWebsite(officialWebsite));
